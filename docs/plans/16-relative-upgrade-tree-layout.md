...
| D12 | ~~Trees are **authored in TS and compiled to JSON at build**~~ — **superseded by D22**: JSON is now the single source of truth, authored via the editor. _(was Open Q G)_                                                                                                                                                                                                                                                                                                                                               |
...
| D17 | The server **owns + serves** the canonical tree files: it loads each one at startup, registers it, and serves the raw bytes at `GET /trees/:mode.json`; both clients fetch from there. _(Phase 5; path moved to `shared/trees/` by D22)_                                                                                                                                                                                                                                                                                |
...
| D19 | ~~The emitted `server/trees/idler.json` is **committed**, with a byte-identical **drift-guard test**; `pnpm emit:trees` regenerates it manually~~ — **superseded by D22**: the JSON is hand/editor-authored, so there is no emit step or drift guard. _(Phase 5)_                                                                                                                                                                                                                                                       |
...
| D22 | **Reverses D12/D19**: the JSON is the **single source of truth**, authored via the dev-page editor — not emitted from TS. The canonical file lives at **`shared/trees/idler.json`** (owned by `@game/shared`, exposed via its `exports` map); the server resolves + serves it, and the dev page + tests `import` it. The TS authoring tree, `toTreeFile`/`buildIdlerTreeFile`, the `emit:trees` script, and the drift-guard test are removed; the only guard is a "parses into a valid runtime tree" check. _(Phase 6)_ |
...
> **Superseded (source-of-truth inversion; D22):** once the editor could author trees, **D12/D19
